...
┌─────────────────────────────────────────────────────────────────────────┐
│  INGEST LAYER                                                           │
│  Amazon MSK (Kafka) or SQS — topics: customers-delta, cases-delta       │
│  • Retention x days (replay/RPO buffer) dependes on the reqs            │
└───────┬───────────────────────────────────────────────────────┬─────────┘
        │                                                       │
        ▼                                                       ▼
┌────────────────────┐                              ┌──────────────────────┐
...
3. **MSK or SQS + DMS data transfer** — inter-AZ traffic costs at 5K updates/sec. Control: use MSK tiered storage for older offsets; compress Kafka messages (LZ4); place DMS replication instance in same AZ as Aurora primary.
